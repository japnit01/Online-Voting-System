...
def reg_server(root,frame1,voter_ID,name,sex,passw):
    if(passw=='' or passw==' '):
        msg = Message(frame1, text="Error: Missing Fileds", width=500)
        msg.grid(row = 10, column = 0, columnspan = 5)
        return -1

    vid = df.taking_data_voter(voter_ID,name,sex,passw)
...
def Register(root,frame1):
    ...
    # Label(frame1, text="Roll No:         ", anchor="e", justify=LEFT).grid(row = 3,column = 0)
    ...
    e4.current()
# , zone.get(), city.get()
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1,voter_ID.get(), name.get(), sex.get(), password.get()), width=10)
