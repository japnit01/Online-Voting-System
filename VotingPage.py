...
def votingPg(root,frame1,client_socket):
    ...
    Radiobutton(frame1, text = "\nSaurabh Kumar Jha", variable = vote, value = "bjp", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"bjp",client_socket)).grid(row = 2,column = 1)
    # bjpLogo = ImageTk.PhotoImage((Image.open("img/bjp.png")).resize((45,45),Image.ANTIALIAS))
    bjpImg = Label(frame1).grid(row = 2,column = 0)

    Radiobutton(frame1, text = "\nSarthak Arora", variable = vote, value = "cong", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"cong",client_socket)).grid(row = 3,column = 1)
    # congLogo = ImageTk.PhotoImage((Image.open("img/cong.jpg")).resize((35,48),Image.ANTIALIAS))
    congImg = Label(frame1).grid(row = 3,column = 0)

    Radiobutton(frame1, text = "\nSaraansh Aggarwal", variable = vote, value = "aap", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"aap",client_socket) ).grid(row = 4,column = 1)
    # aapLogo = ImageTk.PhotoImage((Image.open("img/aap.png")).resize((55,40),Image.ANTIALIAS))
    aapImg = Label(frame1).grid(row = 4,column = 0)

    Radiobutton(frame1, text = "\nNeha Sharma", variable = vote, value = "ss", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"ss",client_socket)).grid(row = 5,column = 1)
    # ssLogo = ImageTk.PhotoImage((Image.open("img/ss.png")).resize((50,45),Image.ANTIALIAS))
    ssImg = Label(frame1).grid(row = 5,column = 0)
    ...
    # notaImg = Label(frame1).grid(row = 6,column = 0)
